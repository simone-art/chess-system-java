...
public class ChessMatch {
	...
	public ChessPiece replacePromotedPiece(String type) {
		if(promoted == null) {
			throw new IllegalStateException("There is not pice to be promoted");
		}
		if(!type.equals("B") && !type.equals("N") && !type.equals("R") && 
				!type.equals("Q")) {
			// retorna a peça que está lá
			return promoted;
		}
	...
	//Método auxiliar que instancia peça específica
	...
	private Piece makeMove(Position source, Position target) {
		...
		if (p instanceof Pawn) {
			if (source.getColumn() != target.getColumn() && capturedPiece == null) {
				Position pwanPosition;
				if (p.getColor() == Color.WHITE) {
					pwanPosition = new Position(target.getRow() + 1, target.getRow());
				} else {
					pwanPosition = new Position(target.getRow() - 1, target.getRow());
				}
				// Remove o peão do tabuleiro
				capturedPiece = board.removePiece(pwanPosition);
				capturedPieces.add(capturedPiece);
				piecesOnTheBoard.remove(capturedPiece);
			}
		}
...
//		board.placePiece(p, source);
		...
		if (p instanceof Pawn) {
			if (source.getColumn() != target.getColumn() && capturedPiece == enPassantVulnerable) {
				...
				Position pwanPosition;
				if (p.getColor() == Color.WHITE) {
					pwanPosition = new Position(3, target.getRow());
				} else {
					pwanPosition = new Position(4, target.getRow());
				}
				board.placePiece(pawn, pwanPosition);
				// Remove o peão do tabuleiro
//				capturedPiece = board.removePiece(pwanPosition);
//				capturedPieces.add(capturedPiece);
//				piecesOnTheBoard.remove(capturedPiece);
	...
	// Método que verifica se existe uma peça adversária que possa fazer um
	// movimennto de check
